...
class Operations:
    ...
    def restart_kubelet(cls, node_name, interval_time):
        ...
        if globals.K8S_DISTRO == 'rke2':
            ...
            restart = 'sudo systemctl restart rke2-server.service'
        elif globals.K8S_DISTRO == 'rke1':
            stopCmd = 'sudo docker stop kubelet'
            startCmd = 'sudo docker run kubelet'
            restart = 'sudo docker restart kubelet'
        elif globals.K8S_DISTRO == 'k3s':
            stopCmd = 'systemctl stop k3s-agent.service'
            startCmd = 'systemctl start k3s-agent.service'
            restart = 'systemctl restart k3s-agent.service'        
        else:
        ...
        if int(interval_time) == 0:
            for instance in node_instances: 
                ...
                print(f'ip_address:{ip_address}, cmd:{restart}')
                Utility.ssh_and_exec_cmd(ip_address, restart)
        else:     
            for instance in node_instances: 
                ip_address = instance.public_ip_address
    ...
    def interrupt_network(cls, node_name, time_interval, volume_end_point):
        ...
        print(f'endpoint:{volume_end_point}')
        print('start generate execute_commands')
        execute_commands = [f'sudo dd if=/dev/urandom of={volume_end_point} bs=2M count=2048 &']
        ...
        print(f'end generate execute_commands:{execute_commands}')
    ...
    def write_random_data_v2(cls, node_name, size):
        print(f'[{datetime.datetime.now()}] write_random_data: {node_name} for {size} MB')
        ...
        for instance in node_instances:
            ip_address = instance.public_ip_address
            print(f'[{datetime.datetime.now()}] write data into {ip_address} network')
            Utility.ssh_and_exec_cmd(ip_address, '\n'.join(execute_commands))   
