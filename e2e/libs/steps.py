...
class steps:
    ...
    def get_pod_state(self, pod_list):
        pods = pod_list.items
        if len(pods) == 0:
            raise Exception("empty pod list")

        logging.info(f"getting the pods {pods} states")
        pods_states = {}
        for pod in pod_list.items:
            pod_name = pod.metadata.name
            pod_state = pod.status.phase
            logging.debug(f"pod {pod_name} state {pod_state}")
            pods_states[pod_name] = pod_state
        return pods_states
    ...
    def disconnect_network(self, node_index):
        ...
        logging.info(f'disconnecting the node: {node_name} network')
    ...
    def connect_network(self, node_index):
        ...
        logging.info(f'connecting the node {node_name} network')
    ...
    def write_volume_random_data_V2(self, node_index, size_in_mb):
        ...
        logging.info(f"writing {size_in_mb} Mb data into the node {node_name}")
