...
class steps:
    ...
    def attach_volume_without_replica(self, volume_name, expect_replica_count):
        print('attach_volume_without_replica')
        node_index = self.get_no_replica_node(volume_name, expect_replica_count)
        
        if node_index != None:
            self.attach_volume(volume_name, node_index[0])
        else:
    ...
    def reboot_node(self, node_index):
        print("reboot_node")
        node_name = Nodes().get_by_index(node_index)
        print(f'target node name: {node_name}')
    ...
    def get_no_replica_node(self, volume_name, expected_replica_count):
        ...
        for node in nodes:
            ...
            if len(replica["items"]) == 0:
                ...
                no_replica_nodes.append(Nodes().get_index_by_name(node))
    ...
    def get_with_replica_node(self, volume_name, expected_replica_count):
        ...
        for node in nodes:
            ...
            print(f'replica:{replica["items"]}, type: {type(replica["items"])}')       
            if len(replica["items"]) != 0:
                print(f'node with replica: {node}')
                with_replica_nodes.append(Nodes().get_index_by_name(node))
                
        if len(with_replica_nodes) == 0:
            raise Exception(f"can not find node with replica: {node}")
